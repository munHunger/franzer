...
const detectAndComputeAsync = (det, img) =>
  det
    .detectAsync(img)
    .then((kps) => det.computeAsync(img, kps).then((desc) => ({ kps, desc })));

const img1 = cv.imread("../data/flower1.jpg");
const img2 = cv.imread("../data/flower2.jpg");
...
const promises = detectorNames.map(createDetectorFromName).map((det) =>
  ...
  Promise.all([
    detectAndComputeAsync(det, img1),
    detectAndComputeAsync(det, img2),
  ]).then((allResults) =>
    cv
      .matchBruteForceAsync(allResults[0].desc, allResults[1].desc)
      .then((matches) => ({
        matches,
